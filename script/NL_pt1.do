...
// load data
insheet using "$path/output/processed/NL_processed.csv", clear 
...
gen p_nl_1 = ((school_closure + work_from_home + no_private_gathering) > 0)
gen p_nl_2 = no_public_gathering
gen p_nl_3 = home_isolation
gen p_nl_4 = travel_ban_local
gen p_nl_5 = travel_ban_intl
  
lab var p_nl_1 "School clos, work home, no private gathering"
lab var p_nl_2 "No public gathering"
lab var p_nl_3 "Home isolation"
lab var p_nl_4 "Travel ban domestic"
lab var p_nl_5 "Travel ban international"

  
//------------------main estimates

// output data used for reg
outsheet using "$path/output/regression/NL_reg_data_10.csv", comma replace

// main regression model
reghdfe D_l_cum_confirmed_cases p_nl_*, absorb(i.adm1_id i.dow, savefe) cluster(t) resid


tempfile results_file
postfile results str18 adm0 str50 policy beta se using `results_file', replace

foreach var in "p_nl_1" "p_nl_2" "p_nl_3" "p_nl_4" "p_nl_5" {
	local varlabel : variable label `var'
	post results ("NL") ("`varlabel'") (round(_b[`var'], 0.001)) (round(_se[`var'], 0.001)) 
}

lincom p_nl_1 + p_nl_2 + p_nl_3 + p_nl_4 + p_nl_5

post results ("NL") ("combined policy") (round(r(estimate), 0.001)) (round(r(se), 0.001)) 
...
sum y_counter
post results ("NL") ("no_policy_rate") (round(r(mean), 0.001)) (round(r(sd), 0.001)) 

postclose results
preserve
	use `results_file', clear
	outsheet * using "$path/output/regression/NL_forest_coefs_1.csv", comma replace
restore
